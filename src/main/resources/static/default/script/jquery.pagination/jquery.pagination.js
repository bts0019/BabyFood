    ...
    return this.each(function () {
        ...
        */
        function getInterval() {
            var neHalf = Math.ceil(opts.num_display_entries / 2);
            var np = numPages();
            var upperLimit = np - opts.num_display_entries;
            var start = currentPage > neHalf ? Math.max(Math.min(currentPage - neHalf, upperLimit), 0) : 0;
            var end = currentPage > neHalf ? Math.min(currentPage + neHalf, np) : Math.min(opts.num_display_entries, np);
            return [start, end];
        }
        ...
        */
        function drawLinks() {
            ...
            var appendItem = function (pageId, appendopts) {
                ...
                if (pageId == currentPage) {
                    ...
                    lnk = jQuery("<a>" + (appendopts.text) + "</a>")
                        ...
                        .attr('href', opts.link_to.replace(/__id__/, pageId));
                }
            ...
            if (opts.prev_text && (currentPage > 0 || opts.prev_show_always)) {
                appendItem(currentPage - 1, { text: opts.prev_text, classes: "prev" });
            }
            // 产生起始点
            var i;
            ...
            // 产生 "Next"-链接
            if (opts.next_text && (currentPage < np - 1 || opts.next_show_always)) {
                appendItem(currentPage + 1, { text: opts.next_text, classes: "next" });
            }
            //总记录数
            jQuery("<span>" + opts.showTotal.replace("{total}", (parseInt(maxentries))) + "</span>").appendTo(panel);
        ...
        drawLinks();
        // 回调函数
        opts.callback(currentPage, this);
    });
}
